...
def main():
    ...
    if choice == menu[0]:
        ...
        beta = st.number_input('Beta - parâmetro de forma da distribuição de probabilídade de Weibull para o tempo até a falha', step = 0.0000001)
        eta = st.number_input('Eta - parâmetro de escala da distribuição de probabilídade de Weibull para o tempo até a falha', step = 0.0000001)    
        lbda = st.number_input('Lambda - taxa de chegada de oportunidades para manutenção', step = 0.0000001)
        cp = st.number_input('Cp - custo de substituição preventiva em T (programada)', step = 0.0000001) 
        cv = st.number_input('Cv - custo de substituição preventiva em Z (prorrogada)', step = 0.0000001)
        co = st.number_input('Co - custo de substituição preventiva antecipada por oportunidade', step = 0.0000001) 
        cw = st.number_input('Cw - custo de substituição preventiva em oportunidade posterior a T', step = 0.0000001)
        cf = st.number_input('Cf - custo de substituição corretiva', step = 0.0000001) 
        p = st.number_input('P - probabilidade de impedimento para substituição preventiva na data programada', step = 0.0000001)
